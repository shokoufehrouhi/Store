...
function getUploadMiddleware() {
  ...
  const storage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
      ...
      if (!ext) {
        const mimeExt = { 'image/jpeg':'.jpg','image/png':'.png','image/gif':'.gif',
  ...
  return multer({
    ...
    fileFilter: (req, file, cb) => {
      ...
      if (!okMime && !okExt) {
        return cb(Object.assign(new Error('نوع فایل پشتیبانی نمی‌شود'), { status: 400 }), false);
      }
      cb(null, true);
    },
  });
...
module.exports = { getUploadMiddleware, uploadDir };
